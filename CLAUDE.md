...
**Cross-cutting traps from robust-bash-and-cancel arc (2026-06-04,
Layer 1+2+5 fragments, `docs/plans/2026-06-04-robust-bash-and-cancel.md`).**
Three structural invariants landed alongside the cancel-finalisation
fix from run `20260604-201957-62d5`:

- **`PiHarness.spawn` MUST pass `start_new_session=True` and
  `PiSession.cancel` MUST use `os.killpg`.** The two are paired:
  Layer 1 (pi as session leader, pgid == pid) without Layer 2 (group
  signal) leaves descendants (vite, npm, http.server) holding pi's
  stdout fd open and `PiSession.events()`'s `async for raw in
  self._proc.stdout` never EOFs. The run row stays 'running' until
  container restart (ADR-31 orphan sweep is the only recovery). Any
  future harness work that refactors `spawn` or `cancel` MUST preserve
  both. Known escape: a descendant that itself calls `setsid()`
  escapes the cascade — none of pi v0.78.0's tools do this today; if
  observed, walk `/proc/<pid>/task/<tid>/children` and SIGKILL each.

- **`StreamEvent.ts: string | undefined` is a dual-path contract.**
  The SSE ingest path (`stores/events.ts:onSseEvent`) reads
  `envelope.ts`; the REST replay path (`loadReplay`) reads `r.ts`.
  Both must keep populating it — `ToolCallCard`'s "running Ns" chip
  reads `tool_use_start.ts` to anchor the elapsed-time clock, and a
  refresh into REST replay must match the live-stream behaviour. If
  the SSE envelope shape (`api/events.py:_event_payload`) or
  `EventOut` REST shape drops `ts`, the chip silently disables for
  one of the two paths.

- **`RunHealthBadge` anchors state on `lastEventTs`, not heartbeat
  `receivedAt`.** Heartbeats arrive every `_KEEPALIVE_S=5s` regardless
  of pi activity (ADR-45), so a `receivedAt`-only classifier reports
  "live" through any silence (the bug from run
  `20260604-201957-62d5`: 8 minutes of false "live"). The fifth
  `disconnected` state is for SSE itself dying — `nowMs - receivedAt
  > HEARTBEAT_GAP_MS=20_000` (4x cadence, 3 missed heartbeats of
  grace). If the backend `_KEEPALIVE_S` changes, `HEARTBEAT_GAP_MS`
  MUST move with it; coupled comment on
  `frontend/src/components/runs/RunHealthBadge.vue` references
  `api/events.py`.

**Deferred to follow-up PRs** (paired with pi-fork retags
`relay-bridge-v2` / `relay-bridge-v3`): Layer 3 (foreground `bash`
timeout), Layer 4 (`bash_background` tool family), Layer 5 skill
text + `RELAY_BG_REMINDER:` preamble line. See the design doc.
