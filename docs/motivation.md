...
3. **Dashboard as primary control plane.** The dashboard is the default UI for everything: browsing projects, reading prompts and plans (rendered properly — markdown, diagrams, code), reviewing artifacts, starting/pausing/cancelling runs, *and editing reviewable artifacts mid-pause* (the 14a–14f review pane: the agent declares `review_path="..."` on a `pause-for-input` sentinel, the dashboard renders an inline editor, the operator's edits are saved via the single sandboxed `PUT /api/runs/:id/artifacts/{path}` endpoint, and the resumed iter picks them up). The "scary commit without preview" of the CLI default is replaced with **browse-render-review-edit-resume**. The CLI remains as a scripting/automation surface but is no longer the assumed default.
