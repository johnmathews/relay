...
| `harness_session_ended` | iter's harness session terminates (every close path) — appended **before** `iter_ended` (spec §6, ADR-39) | `{stop_reason, messages, summary, exit_reason}` — `stop_reason ∈ {clean, crash, timeout, cancelled}`; `messages` is `SessionEnded.messages` verbatim (ADR-18 opaque); `summary` populated only on the `done` close path, `null` otherwise; `exit_reason ∈ {signal, cancelled, timeout, agent_end_no_signal, crash}` mirrors the paired `iter_ended.exit_reason` so a UsageRow renderer can soft-relabel pi's `stop_reason="cancelled"` on a normal signal-closed iter (ADR-48). Old replays predate `exit_reason` — renderers must accept its absence and fall back to `stop_reason`. |
