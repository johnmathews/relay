...
- **Editable / source install** (``uv sync`` + ``uv run`` on a checkout):
  the package is at ``src/relay/``; the skill tree is the repo-root
...
- **Built wheel**:
  ...
  ``relay/skills/``, so the skill ships *inside* the package at
...
def _candidates(harness: str) -> tuple[Path, Path]:
    ...
    pkg_root = Path(__file__).resolve().parent.parent  # …/relay
    # parents: [0]=harness [1]=relay [2]=src [3]=<repo root>
