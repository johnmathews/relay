...
Each tool maps 1:1 to a :class:`~relay_v2.core.RelayCore` method and
returns the *same* Pydantic schema the REST API uses
(:mod:`relay_v2.api.schemas`), so MCP and REST stay consistent by
...
def create_mcp_server(core: RelayCore, settings: Settings) -> FastMCP:
    ...
    mcp: FastMCP = FastMCP("relay-v2", streamable_http_path="/")
